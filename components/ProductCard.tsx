...
export default function ProductCard({
  id,
  title,
  location, // ✅ مصححة: كائن متداخل بدل governorate/district مباشرة
  price_per_hour,
  icon,
  primary_image,
  status,
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex flex-col items-start text-right">
        <h3 className="font-bold text-gray-800 text-sm mb-1 line-clamp-1">{title}</h3>

        <div className="flex items-center gap-1 text-gray-400 text-xs mb-3">
          <span className="material-symbols-rounded text-xs text-primary">location_on</span>
          {/* ✅ مصححة: location.governorate / location.district بدل الحقول المباشرة */}
          <span>{location.governorate} — {location.district}</span>
        </div>
        ...
        <span
          className={`w-full py-2.5 rounded-btn font-bold text-sm text-center transition-all ${
            isAvailable
