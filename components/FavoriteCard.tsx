...
export default function FavoriteCard({ item }: { item: PublicProduct  }) {
  ...
  {item.icon || getCategoryIcon(item.category)}
      ...
      <div className="flex-1 min-w-0 text-right">
        <h3 className="font-bold text-gray-800 text-xs truncate">{item.title}</h3>
          <p className="text-[10px] text-gray-400 mt-0.5">{item.location.governorate} — {item.location.district}</p>
            <div className="text-primary font-black text-[11px] mt-1">₪{item.price_per_hour}/ساعة</div>
      </div>
      <div className="flex gap-1.5">
        <button
          onClick={() => toggleFavorite(item.id)}
          className="p-1.5 rounded-lg text-red-400 border border-red-50 hover:bg-red-50 transition-all"
        >
